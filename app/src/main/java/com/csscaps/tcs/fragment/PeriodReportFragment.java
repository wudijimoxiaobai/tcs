...
import com.csscaps.tcs.dialog.ReportDailyReportFragmentDialog;
...
public class PeriodReportFragment extends BaseFragment implements AdapterView.OnItemClickListener {
    ...
    private ReportDailyReportFragmentDialog reportDailyReportFragmentDialog;

    @Override
    protected int getLayoutResId() {
        return R.layout.period_report_fragment;
    }
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.id_export_item:
                        ...
                        String fileName = file.toString() + "/" + "PeriodReport" + time + ".xls";
                        ...
                        datePickerDialogFragment.setOnDateChooseListener(new DatePickerDialogFragment.OnDateChooseListener() {
                            ...
                            public void onDateChoose(String s, String s1) {
                                ...
                                if (Integer.parseInt(s.replace("-", "")) <= Integer.parseInt(s1.replace("-", ""))) {
                                    years1 = s;
                                    years2 = s1;
                                    List<PeriodReportSection> listPrs = getFinalData(years1, years2);
                                    mPeriodReportAdapter = new PeriodReportAdapter(mContext, R.layout.period_report_list_item, listPrs);
                                    mListView.setAdapter(mPeriodReportAdapter);
                                    periodReportAdapter2 = new PeriodReportAdapter2(mContext, R.layout.period_report_list_item2, listPrs);
    ...
    private List<PeriodReportSection> getFinalData(String s, String s2) {
        int no = 0;
...
//        DailyReportFragment dailyReportFragment = DailyReportFragment.newInstance(listPrs.get(i).getPeriod());
//        FragmentTransaction transaction = getFragmentManager().beginTransaction();
//        transaction.add(R.id.content, dailyReportFragment);
//        transaction.addToBackStack(null);
//        transaction.commit();
        String period = listPrs.get(i).getPeriod();
        if (reportDailyReportFragmentDialog == null)
            reportDailyReportFragmentDialog = new ReportDailyReportFragmentDialog(period);
        reportDailyReportFragmentDialog.show(getFragmentManager(), "ReportDailyReportFragmentDialog");
    }
}