...
public class OnlineDeclarationAdapter extends QuickAdapter<ControlData> {
    ...
    protected void convert(BaseAdapterHelper helper, ControlData item, int position) {
        ...
        helper.setText(R.id.deadline_date_from,DateUtils.getDateToString_YYYY_MM_DD_EN(DateUtils.getStringToDate(item.getS_report_date(),DateUtils.format_yyyyMMdd)));
