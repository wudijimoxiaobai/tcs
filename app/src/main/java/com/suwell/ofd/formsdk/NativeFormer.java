...
public class NativeFormer {
    ...
    static {
        ...
        System.loadLibrary("swsiqnwrapper");
    }


    private NativeFormer openZIP(String pkg) throws FormerException {
        open(pkg, 0);
        return this;
    }

    private NativeFormer openXML(String xml) throws FormerException {
        open(xml, 1);
        return this;
    }
    ...
    private NativeFormer parse() throws FormerException {
        checkClosed();
        checkOpen();
        if (Parse(this.uid) != 0) {
    ...
    private NativeFormer save(String ofd) throws FormerException {
        checkClosed();
        checkOpen();
        if (!this.parsed) {
